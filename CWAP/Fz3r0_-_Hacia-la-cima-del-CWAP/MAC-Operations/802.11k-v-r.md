...
- `Reassociation Request` :: Sent from: `STA` :: `wlan.fc.type_subtype == 2 && wlan.pmkid.akms`
- `EAPOL M1` :: Sent from: `AP` :: `wlan_rsna_eapol.keydes.msgnr == 1 && wlan.rsn.ie.pmkid`
- `EAPOL M2` :: Sent from: `STA` :: `wlan_rsna_eapol.keydes.msgnr == 2 && wlan.pmkid.akms`
...
- PMK Caching: **The PMKID and PMKSA are specific to each client-AP pairing**, and if a client roams to a new AP, a full authentication process is required.
- OKC: **A single PMK can be shared among APs connected to the same controller**, allowing the client to roam across APs with only the initial 802.1X/EAP authentication required once. Subsequent roaming only requires the 4-Way Handshake if the PMKID matches.
