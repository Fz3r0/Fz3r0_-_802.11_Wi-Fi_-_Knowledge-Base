...
2. 802.1X Slow Roaming
3. 802.1X PMK Caching (PMKID) :: Fast-Roam-Back - 
4. 802.1X Preauthentication
5. 802.1X OKC (Opportunistic Key Caching)
...
Preauthentication is generally considered a legacy method for secure roaming in Wi-Fi networks. While it was a valuable approach in early wireless networks, preauthentication has largely been replaced by more efficient and scalable roaming solutions, particularly 802.11r (Fast BSS Transition)
...
How Preauthentication Is Advertised
If an AP supports preauthentication, it advertises this capability in its beacon frames using a field called the RSN Information Element (RSNIE). This way, client devices know they can initiate preauthentication with that AP, ensuring faster transitions if they roam.
Benefits and Drawbacks of Preauthentication
Benefits:

Preauthentication significantly reduces reauthentication time when a client roams to a new AP, as the secure session is already established. The process only requires a 4-Way Handshake to complete the connection, leading to faster roaming.
Drawbacks:
...
Due to the scalability challenges of both PMK Caching and Preauthentication, the IEEE ratified the 802.11r-2008 standard, also known as Fast BSS Transition (FT), to address these issues. Before 802.11r, some vendors implemented Opportunistic Key Caching (OKC), an alternative that allowed clients to reuse PMKs across multiple APs within the same network without needing to authenticate with each one individually.

Summary of Preauthentication Process
Client Preparation: Preauthentication lets a client prepare for future APs by establishing a PMKSA with a target AP in advance.
Seamless Transition: If the client roams to the target AP, it can skip 802.1X/EAP and proceed directly to the 4-Way Handshake.
Limitations: Scalability can be challenging, as each client needs to authenticate with every potential AP, creating load on the AS.
Preauthentication was an important step in improving roaming speeds, but the challenges it posed led to the development of more scalable solutions, such as 802.11r, for faster and more efficient wireless roaming.
