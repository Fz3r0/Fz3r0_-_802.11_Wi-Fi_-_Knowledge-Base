...
1. **802.11k (RRM / assisted roaming / neighbor report) informs clients for roaming decisions whereas 802.11v (WNM / BSS Transition) manages roaming of the client STA based on AP determination (AP use information from 802.11k).**
2. **If 802.11r (FT) is used with 802.11k/v (RRM/WNM), in addition of point 1., the client STA will roam in less than 50ms if the client is using 802.1X EAP (the client will roam using only 4-way-handshake without EAP exchange)**
