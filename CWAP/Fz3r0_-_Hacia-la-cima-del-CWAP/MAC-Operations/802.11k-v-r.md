...
- **Once the STA is authenticated via 802.1X/EAP on AP-1 and has its PMK-1, if it wants to roam to AP-2 (which it has never been authenticated on before), it first notifies AP-1. AP-1 then sends the RSNIE containing PMK-2 to AP-2, all through the Ethernet network, where both APs can "see" each other.**
