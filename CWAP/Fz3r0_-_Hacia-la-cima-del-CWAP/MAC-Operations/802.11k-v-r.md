...
Some Vendors such as Cisco or Ruckus extend the PMKID key caching mechanism to pro-actively cache the keys in a Wi-Fi BSS network. This mechanism is also termed as `Opportunistic Key Caching` or `proactive Key Caching` or `opportunistic PMKID caching`.
