...
2. **If `802.11r - {Fast Transition}` is used with `802.11k/v (RRM/WNM)`, in addition of point 1., the client STA will `roam in less than 50ms` if the client STA is using `802.1X EAP` _(the client will roam using only 4-way-handshake without EAP exchange)_-**
