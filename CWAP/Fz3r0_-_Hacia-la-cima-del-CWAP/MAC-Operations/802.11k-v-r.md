...
- `802.11v` - `Wireless Network Management (WNM)` :: `{BSS Transition - [BSS Transition Management req/res]}` :: **MANAGE** the client STA roaming via **BSS Transition Management** through **BSS Transition Management requests/responses**. Here, the **AP uses information from 802.11k (RRM)** to guide the client’s roaming.

If **802.11r** - `Fast Transition (FT)` is used alongside `802.11k/v (RRM & WNM)`:

- In addition to the benefits of 802.11k and 802.11v, the `client STA can roam in under 50 ms` **(useful for VoIP networks)** if using 802.1X EAP. In this case, the client STA only needs the 4-way handshake, with no full EAP exchange required.
...
1. **`802.11k - RRM  {assisted roaming - [neighbor req/res]}` INFORMS client STAs for roaming decisions, whereas `802.11v - WNM {BSS Transition - [BSS Transition Management req/res]}` MANAGES ROAMING of the client STA based on AP determination _(AP use the information from 802.11k)_.** <br><br>
2. **If `802.11r - {Fast Transition}` is used with `802.11k/v (RRM/WNM)`, in addition of point 1., the client STA will `roam in less than 50ms` _(usefull for VoIP networks)_ if the client STA is using `802.1X EAP` _(the client will roam using only 4-way-handshake without EAP exchange)_-**

`Important`: **802.11r (FT) is not really needed in WPA2 PSK networks**, because that kind of RSNA does not need to call a RADIUS server from WAN. **The roaming in a WPA2 network using the 4-way-handshake is only between APs & STA which is already 50ms or less.** 
...
**IEEE 802.11k-2008** is an amendment to IEEE 802.11-2007 standard for `Radio Resource Measurement (RRM)`. It is also known for `802.1k Assisted Roaming` purposes, but is not the only one.  

- RRM defines and exposes **radio and network information** to **facilitate the management and maintenance of a WLAN**
...
- **802.11k - RRM** inform client STAs of the WLAN RF information such as the AP **Channel** and **RSSI**.
- **Neighbor Report Requests (Action)** & **Neighbor Report Responses (Action)** exchange this info between STAs, that's why those action frames are tipically used in 802.11k. 

802.11k allows clients to make more informed roaming decisions. Ultimately, **client STAs still make the final roaming decision.** 
...
RRM Information Element is present in:
