...
### Enterprise (WPA2/WPA3 + 802.1X-EAP)
...
- PMKSA (Pairwise Master Key Security Association): When a client device (such as a phone or laptop) initially connects to an AP, it undergoes a secure authentication process, creating a session key called the PMK (Pairwise Master Key). This session is represented as a PMKSA.
- PMK Caching: After this initial connection, both the client and the AP store (or "cache") the PMKSA for a set period. This allows them to reuse the PMK later if the client returns to the same AP, avoiding the need for full authentication again.
...
- The client connects to `AP-1`, known as the **"original AP"**, and completes a full 802.1X/EAP authentication (4 State Machine = EAP/RADIUS + 4-way-handshake).
- This process creates an **initial PMK**, referred to as `PMK-1`.
- **Both the `client STA` & `AP-1` cache this `PMK-1` for potential future use.**

#### 2. Roaming to a New AP:

When the client moves to a new area, it may connect to a target AP (a different AP in the network). In this case, the client and target AP perform another full 802.1X/EAP authentication, creating a new PMK, PMK#2.
The client and target AP cache PMK#2 for future use if the client returns to the target AP.
Roaming Back to the Original AP (Using PMK Caching):

Later, if the client roams back to the original AP (the first AP it connected to), it can use the PMK#1 cached on both devices.
In this case, the client includes a list of PMKIDs (identifiers for cached PMKs) in its Reassociation Request to the original AP. This tells the AP that it has cached PMKs for faster reauthentication.
The original AP, recognizing the PMKID from PMK#1, skips the full 802.1X/EAP authentication and instead proceeds directly with the 4-Way Handshake. This step quickly re-establishes the secure connection between the client and AP.
