...
- **Channel Switch Mode**: (Most of the time will be 1) - AP tells the STA that not only they need to change channel, it also says they must stop Transmit at that moment. Even if the window could be 10 seconds to change the channel, the STA must stop Transmitting just at the moment the CSA is received.
- **New Channel Number**: Inform all the client STAs which will be the new channel.
