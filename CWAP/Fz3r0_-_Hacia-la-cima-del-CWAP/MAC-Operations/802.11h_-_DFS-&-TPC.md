...
|----------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------|
| **`CAC (Channel Availability Check)`** | DFS function that monitors a channel to determine if a RADAR waveform above the DFS detection threshold is present. `CAC Time` is the time for which the system must monitor a channel for radar prior to any data being transmitted over the channel.                                                                                                                           | Normal DFS: _(max)_ **60 seconds**<br>Weather DFS: _(max)_ **600 seconds** |
