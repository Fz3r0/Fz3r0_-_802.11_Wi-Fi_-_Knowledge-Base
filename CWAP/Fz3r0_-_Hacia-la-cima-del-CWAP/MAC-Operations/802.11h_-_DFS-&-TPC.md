...
- **Channel Switch Mode**: Instructions from AP to STA (Most of the time will be 1 = Silence). AP tells the STA that not only they need to change channel, it also says they must stop Transmit at that moment. Even if the window could be 10 seconds to change the channel, the STA must stop Transmitting just at the moment the CSA is received.
- **New Channel Number**: New channel to "jump". Inform all the client STAs which will be the new channel.
- **Channel Switch Count**: Count Down to the channel "jump". Number of TBTTs before the channel jump (X microseconds = Y TBTTs). A value of 1 indicated that the channel switch occurs immediately before the next TBTT. 
...
![image](https://github.com/user-attachments/assets/3e1a888e-93f7-4a11-9908-9ac97e92cd0d)
...
![image](https://github.com/user-attachments/assets/38fa5d26-15b5-483a-9538-f5ca6b29331e)
