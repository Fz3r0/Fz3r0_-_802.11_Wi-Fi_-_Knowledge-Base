...
Channel Announcement Element

- All 802.11 frames with channel switch announcement Element - `wlan.tag.number == 37`
- beacon frames with channel switch announcement Element - `(wlan.fc.type == 0 && wlan.fc.type_subtype == 8) && wlan.tag.number == 37`
- probe response frames with channel switch announcement Element - `(wlan.fc.type == 0 && wlan.fc.type_subtype == 5) && wlan.tag.number == 37`
- control frames with channel switch announcement Element - `(wlan.fc.type == 0 && wlan.fc.type_subtype == 13) && wlan.tag.number == 37`

- **Channel Switch Mode**:  Instructions from AP to STA. If set to 1 AP tells the STA that not only they need to change channel, it also says they must stop Transmit at that moment. Even if the window could be 10 seconds to change the channel, the STA must stop Transmitting just at the moment the CSA is received. If set to 0 the client STA can keep transmiting until the jump - filer `wlan.csa.channel_switch_mode == 0`
- **New Channel Number**: New channel to "jump". Inform all the client STAs which will be the new channel (eg. Channel 36). - filer `wlan.csa.new_channel_number == 36`
- **Channel Switch Count**: Count Down to the channel "jump". Number of TBTTs before the channel jump (eg. 10 Beacons before jump / 10 TBTTs). A value of 1 indicated that the channel switch occurs immediately before the next TBTT. - filter `wlan.csa.channel_switch.count == 10`

Same components but beacon only:

`(wlan.fc.type == 0 && wlan.fc.type_subtype == 8) && wlan.csa.channel_switch_mode == 0`
`(wlan.fc.type == 0 && wlan.fc.type_subtype == 8) && wlan.csa.new_channel_number == 36`
`(wlan.fc.type == 0 && wlan.fc.type_subtype == 8) && wlan.csa.channel_switch.count == 10`

Same components components but probe response only:

`(wlan.fc.type == 0 && wlan.fc.type_subtype == 5) && wlan.csa.channel_switch_mode == 0`
`(wlan.fc.type == 0 && wlan.fc.type_subtype == 5) && wlan.csa.new_channel_number == 36`
`(wlan.fc.type == 0 && wlan.fc.type_subtype == 5) && wlan.csa.channel_switch.count == 10`

Same components  but action:

`(wlan.fc.type == 0 && wlan.fc.type_subtype == 13) && wlan.csa.channel_switch_mode == 0`
`(wlan.fc.type == 0 && wlan.fc.type_subtype == 13) && wlan.csa.new_channel_number == 36`
`(wlan.fc.type == 0 && wlan.fc.type_subtype == 13) && wlan.csa.channel_switch.count == 10`
 


`Important`: If a `client STA` is performing `In-Service Monitoring` and detects a RADAR, it will inform the `AP` using an `Action Frame`. _(This is equivalent to the AP detecting the RADAR waveform)_. 

AP Detection Mechanism:
...
### CSA: `Manual Channel Configuration`

#### From DFS : CH / To

![image](https://github.com/user-attachments/assets/14f3c7fc-90fb-4f48-b8b3-6f5db039655c)


#### From DFS : CH / To

![image](https://github.com/user-attachments/assets/e55240b0-04ad-4f14-8978-4a2bef45152a)


### CSA: `DFS RADAR Detection Event`

![image](https://github.com/user-attachments/assets/459661a6-9430-4585-a33b-15e92402189d)


![image](https://github.com/user-attachments/assets/cc0df7ab-3a83-4bd4-965e-18d5d98618b0)

![image](https://github.com/user-attachments/assets/e2475921-33ca-4040-a2fb-9e7a96bb7039)
...
- https://www.youtube.com/watch?v=wqVUYcHuy4A
