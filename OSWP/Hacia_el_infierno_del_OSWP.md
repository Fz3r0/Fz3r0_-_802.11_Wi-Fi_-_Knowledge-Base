...
El motivo por el cual el sevricio de chromecast no está funcionando y presenta la incidencia de no poder ver el cliente/servidor es porque se ha contenido el tráfico multicast en la red. 

Las chromecast y servicio de Nevaya utiliza tráfico mDNS que es un protocolo multicast, sin el no podrán funcionar correctamente. 

Debido a tormentas multicast provocadas por dispositivos iphone utilizando WiFi calling (protocolo ESP multicast) en la red de huéspedes es que nos hemos visto en la necesidad de limitar por completo el tráfico multicast.

En caso de encender y habilitar este tipo de tráfico, la red se verá comprometida con caídas de los dispositivos de manera aleatoria como APs, switches, controladoras e incluso clientes como huéspedes.
...
# Revisar drivers e información de Interfaz usb (pe. Alfa)
lsusb
